...
export class ReignCombat extends Combat {
  ...
  async nextRound() {
    ...
    await this.unsetFlag("reign", "pendingRolls");
    ...
    await Promise.all(this.combatants.map(async combatant => {
      ...
      if (hadCoverage) {
        actorUpdates["flags.reign.shieldCoverage"] = foundry.data.operators.ForcedDeletion;
      ...
      if (hadDodgeCover) {
        actorUpdates["flags.reign.dodgeCover"] = foundry.data.operators.ForcedDeletion;
      }
      ...
      if (!aimedThisRound) {
        if (actor.getFlag("reign", "aimBonus")) {
          actorUpdates["flags.reign.aimBonus"] = foundry.data.operators.ForcedDeletion;
        }
      }
      if (aimedThisRound) {
        actorUpdates["flags.reign.aimedThisRound"] = foundry.data.operators.ForcedDeletion;
      }
    ...
    const combatantFlagUpdates = this.combatants
      ...
      .map(c => {
        const u = { _id: c.id };
        if (c.getFlag("reign", "shoveBonusAgainst")) u["flags.reign.shoveBonusAgainst"] = foundry.data.operators.ForcedDeletion;
        if (c.getFlag("reign", "ironKissSetup")) u["flags.reign.ironKissSetup"] = foundry.data.operators.ForcedDeletion;
        return u;
      });
