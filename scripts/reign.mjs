...
Hooks.once("init", async () => {
  ...
  const templatePaths = [
    ...
    "systems/reign/templates/apps/combat-dashboard.hbs"
  ];
  ...
  game.reign = {
    ...
    CombatDashboard,                                         // Combat Dashboard class
    openDeclarationDialog: _openDeclarationDialog            // ITEM-8: Advanced declaration dialog
  };
});
...
Hooks.once("ready", () => {
  ...
  // Auto-show when phase changes (declaration → resolution)
  Hooks.on("updateCombat", (combat, changes) => {
    if (changes.flags?.reign?.phase) {
      CombatDashboard.show(dashboard);
    }
  });
});
...
Hooks.on("getSceneControlButtons", (controls) => {
  ...
  const buttonDef = {
    ...
    button: true,
    onChange: () => {
      const dashboard = game.reign?.dashboard;
      if (dashboard) CombatDashboard.toggle(dashboard);
    }
  };
  if (Array.isArray(tools)) {
    tools.push(buttonDef);
  } else if (tools && typeof tools === "object") {
    tools.combatDashboard = buttonDef;
  }
});
...
Hooks.on("deleteCombat", async (combat, options, userId) => {
  ...
  await Promise.all(creatureCombatants.map(async combatant => {
    ...
    const updates = {};
    if (actor.getFlag("reign", "elephantTrumpetUsed")) updates["flags.reign.elephantTrumpetUsed"] = foundry.data.operators.ForcedDeletion;
    if (actor.getFlag("reign", "freeGobbleDice"))      updates["flags.reign.freeGobbleDice"]      = foundry.data.operators.ForcedDeletion;
