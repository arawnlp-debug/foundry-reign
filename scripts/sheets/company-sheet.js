...
export class ReignCompanySheet extends ScrollPreserveMixin(HandlebarsApplicationMixin(foundry.applications.sheets.ActorSheetV2)) {
  ...
  async _onUpgradeQualityXP(event, target) {
    ...
    try {
        ...
        const content = `<p class="reign-dialog-intro">Spend <strong>${cost} XP</strong> to permanently upgrade <strong>${qualityLabel}</strong> to <strong>${newLevel}</strong>?</p>`;
        ...
        await ChatMessage.create({
            ...
            content: `<div class="reign-chat-card reign-card-success">
                <h3 class="reign-msg-success"><i class="fas fa-level-up-alt"></i> Quality Upgraded!</h3>
    ...
    try {
      ...
      const dialogContent = `
        <div class="reign-dialog-form">
          <p class="reign-dialog-intro">Improve ${qualityLabel}</p>
          <p class="reign-dialog-subtitle">
            Target Difficulty: <strong>${currentPerm}</strong> (current permanent rating)<br>
            <em>Failing does NOT grant a temporary increase.</em>
          </p>
          <div class="dialog-grid dialog-grid-2 reign-dialog-section-divider">
              <div class="reign-text-center">
                  <label class="reign-improve-label">Required Q1</label>
                  <strong class="reign-improve-value">${q1Label} (${val1})</strong>
              </div>
              <div class="reign-text-center">
                  <label class="reign-improve-label">Required Q2</label>
                  <strong class="reign-improve-value">${q2Label} (${val2})</strong>
              </div>
          </div>
      ...
      if (successSet) {
        ...
        await ChatMessage.create({
          speaker: ChatMessage.getSpeaker({ actor: this.document }),
          content: `<div class="reign-chat-card reign-card-success">
            <h3 class="reign-msg-success"><i class="fas fa-arrow-up"></i> ${qualityLabel} Improved!</h3>
            ...
            <hr>
            <p class="reign-msg-result">${qualityLabel} permanently increases to ${newPerm}!</p>
          </div>`
        });
        ...
        await ChatMessage.create({
          ...
          content: `<div class="reign-chat-card reign-card-fail">
            <h3 class="reign-msg-fail"><i class="fas fa-times"></i> Improvement Failed</h3>
            <p><strong>${safeName}</strong> rolled <strong>${poolLabel}</strong> (${totalPool}d10): ${results.join(", ")}</p>
            <p>${parsed.sets.length === 0 
                ? "No matched sets." 
                : `Best set ${parsed.sets[0].text} did not meet Difficulty ${currentPerm}.`}</p>
            <hr>
            <p class="reign-text-sm reign-text-muted"><em>${qualityLabel} remains unchanged. ${q1Label} & ${q2Label} eroded.</em></p>
          </div>`
        });
