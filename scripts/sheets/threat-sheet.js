...
export class ReignThreatSheet extends HandlebarsApplicationMixin(foundry.applications.sheets.ActorSheetV2) {
  ...
  _prepareSubmitData(event, form, formData) {
    ...
    for (const key in flatData) {
      if (
        key === "system.threatLevel" ||
        key === "system.magnitude.value" ||
        key === "system.magnitude.max" ||
        key === "system.morale.value" ||
        key === "system.morale.max"
      ) {
        if (flatData[key] === "" || flatData[key] === null) {
          flatData[key] = 0;
          changed = true;
        } else if (typeof flatData[key] === "string" && !isNaN(parseInt(flatData[key], 10))) {
          flatData[key] = parseInt(flatData[key], 10) || 0;
          changed = true;
        }
      }
    }
