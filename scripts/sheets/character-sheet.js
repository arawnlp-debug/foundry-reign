...
export class ReignActorSheet extends ScrollPreserveMixin(HandlebarsApplicationMixin(foundry.applications.sheets.ActorSheetV2)) {
      ...
      try {
          const content = `
            <div class="reign-dialog-form">
              <p class="reign-dialog-intro">Select the type of rest.</p>
  ...
  async _onAcquireLoot(event, target) {
      ...
      try {
          ...
          if (lootValue >= currentWealth) {
              ...
              await ChatMessage.create({ speaker: ChatMessage.getSpeaker({ actor: this.document }), content: `<div class="reign-chat-card reign-card-success"><h3 class="reign-msg-success">Significant Windfall!</h3><p>Acquired loot (Value <strong>${lootValue}</strong>) meets or exceeds current Wealth.</p><hr><p class="reign-msg-result">Permanent Wealth increases to ${newWealth}!</p></div>` });
          } else {
              await ChatMessage.create({ speaker: ChatMessage.getSpeaker({ actor: this.document }), content: `<div class="reign-chat-card"><h3 class="reign-msg-muted">Trivial Loot</h3><p>Acquired loot (Value <strong>${lootValue}</strong>) is below current Wealth (${currentWealth}).</p><hr><p class="reign-text-sm reign-text-muted"><em>It's pocket change and does not affect your permanent standing.</em></p></div>` });
          }
      } catch(err) { ui.notifications.error(`Action failed: ${err.message}`); console.error(err); }
  }
  ...
  async _onUpgradeStat(event, target) {
      ...
      try {
          ...
          const confirm = await reignConfirm("Confirm Advancement", `<p class="reign-dialog-intro">Spend <strong>${cost} XP</strong> to acquire <strong>${upgradeText}</strong>?</p>`);
          if (!confirm) return;
  ...
  async _onItemDelete(event, target) { 
      ...
      try {
          ...
          if (!item) return;
          const confirm = await reignConfirm(`Delete ${item.name}?`, `<p class="reign-dialog-intro">Are you sure you want to permanently delete <strong>${item.name}</strong>?<br>This action cannot be undone.</p>`);
  ...
  async _onItemToChat(event, target) {
      ...
      try {
          ...
          if (item.type === "spell") {
              ...
              if (sys.attunementRequired) metaParts.push(`<span class="reign-spell-chat-locked"><i class="fas fa-lock"></i> Attunement Required</span>`);
              if (metaParts.length) spellMeta = `<div class="reign-callout reign-spell-chat-meta">${metaParts.join(" &nbsp;|&nbsp; ")}</div>`;
          }
          
          await ChatMessage.create({ speaker: ChatMessage.getSpeaker({actor: this.document}), content: `<div class="reign-chat-card"><h3>${safeName}</h3><p>${item.type.toUpperCase()}</p>${spellMeta}<hr><div>${safeDesc}</div></div>` });
      } catch(err) { ui.notifications.error(`Action failed: ${err.message}`); console.error(err); }
  }

  // Collapse/expand a spell school group in the esoterica tab
  _onToggleSpellGroup(event, target) {
      event.preventDefault();
      const group = target.closest(".reign-spell-group");
  ...
  async _onPledgeEffort(event, target) {
      ...
      try {
          ...
          const content = `
            <div class="reign-dialog-form">
              <p class="reign-dialog-intro reign-font-display">
          ...
          await ChatMessage.create({
              ...
              content: `<div class="reign-chat-card reign-card-fail">
                <h3 class="reign-msg-fail"><i class="fas fa-handshake"></i> Effort Pledged!</h3>
                <p>Through hard work, wealth, or blood, <strong>${safeCharName}</strong> has secured an advantage for <strong>${safeCompName}</strong>.</p>
                <hr>
                <p class="reign-msg-result">
  ...
  async _prepareContext(options) {
    ...
    for (const item of items) {
        if (buckets[item.type] !== undefined) {
            ...
            if (item.type === "weapon") {
                ...
                if (item.type === "spell") {
                    if (sys.intensity) tooltip += ` | Intensity: ${sys.intensity}`;
                    if (sys.duration) tooltip += ` | Duration: ${sys.duration}`;
                }
            }
    ...
    // ── SORCERY: Detection radius table (RAW Realms p.147) ─────────────────
    const DETECTION_RADIUS = ["—","—","5 ft","10 ft","50 ft","1,000 ft","1 mile","10 miles","25 miles","50 miles","100 miles"];
    const getDetectionRadius = (intensity) => DETECTION_RADIUS[Math.min(10, Math.max(1, parseInt(intensity) || 1))];
    const intensityClass = (intensity) => {
        const i = parseInt(intensity) || 1;
        if (i <= 2) return "intensity-low";
        if (i <= 4) return "intensity-mid";
        if (i <= 6) return "intensity-high";
        return "intensity-extreme";
    };

    // Annotate each spell with derived display data
    const attunementStatus = system.esoterica?.attunementStatus || "none";
    for (const spell of buckets.spell) {
        spell.detectionRadius    = getDetectionRadius(spell.system.intensity || 1);
        spell.intensityClass     = intensityClass(spell.system.intensity || 1);
        spell.isLocked           = spell.system.attunementRequired && attunementStatus === "none";
        spell.isAttunementSpell  = !!spell.system.isAttunementSpell;
        spell.hasSlow            = (parseInt(spell.system.slow) || 0) > 0;
        spell.hasDuration        = !!(spell.system.duration?.trim());
    }

    // Group spells by school for the esoterica tab
    const schoolMap = new Map();
    for (const spell of buckets.spell) {
        const school = (spell.system.school || "").trim();
        const key    = school || "__ungrouped__";
        if (!schoolMap.has(key)) schoolMap.set(key, { school: school || "Other", isUngrouped: !school, spells: [] });
        schoolMap.get(key).spells.push(spell);
    }
    const spellGroups = Array.from(schoolMap.values())
