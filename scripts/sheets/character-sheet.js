...
export class ReignActorSheet extends HandlebarsApplicationMixin(foundry.applications.sheets.ActorSheetV2) {
    ...
    actions: {
      ...
      acquireLoot: async function(event, target) {
        try {
          ...
          const lootValue = await reignDialog("Acquire Loot", content, (e, b, d) => parseInt(d.element.querySelector('[name="lootValue"]').value) || 0, { defaultLabel: "Evaluate Loot" });
          if (!lootValue) return;
      ...
      upgradeStat: async function(event, target) {
        try {
          ...
          if (type === "attribute") {
            ...
            cost = 5; newPath = isCustom ? `system.customSkills.${key}.master` : (isEsoterica ? `system.esoterica.master` : `system.skills.${key}.expert`); newVal = true; upgradeText = `Master Die for ${label}`;
      ...
      deleteEffect: async function(event, target) {
        ...
        if (effect) { await effect.delete(); this.render(true); }
      },
      toggleEffect: async function(event, target) {
        const effect = this.document.effects.get(target.closest(".effect-item").dataset.effectId);
        if (effect) { await effect.update({ disabled: !effect.disabled }); this.render(true); }
      }
    }
  };
  ...
  async _prepareContext(options) {
    ...
    // AUDIT FIX P2: Health Dual-Loop Consolidated into a single O(N) pass
    context.health = {};
    context.reignHealth = [];
