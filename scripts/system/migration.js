...
function migrateCharacter(actor) {
  ...
  for (const loc of expectedLocations) {
    ...
    // PHASE 5 FIX: Explicitly delete deprecated V11 schema fields to satisfy V13 TypeDataModels
    if (current.max !== undefined) updateData[`system.health.${loc}.-=max`] = null;
    if (current.armor !== undefined) updateData[`system.health.${loc}.-=armor`] = null;
  }
