...
export class XPAwardPanel extends ScrollPreserveMixin(HandlebarsApplicationMixin(ApplicationV2)) {
  ...
  static PARTS = {
    main: { template: "systems/reign/templates/apps/xp-award.hbs" }
  };
  ...
  async _onApplyAwards(event, target) {
    ...
    const confirmed  = await foundry.applications.api.DialogV2.confirm({
      window:   { title: "Confirm XP Awards", classes: ["reign-dialog-window"] },
      ...
      content: `<div class="reign-dialog-form">
        ...
        <p class="reign-text-small reign-text-muted">Each character's unspent XP will increase by their individual total. This cannot be undone automatically.</p>
      </div>`,
      rejectClose: false,
    });
    if (!confirmed) return;
    ...
    await ChatMessage.create({
      ...
      content: `<div class="reign-chat-card reign-card-success">
        <h3 class="reign-msg-success"><i class="fas fa-award"></i> Session XP Awarded</h3>
        <div class="xa-chat-list">${rows}</div>
        <p class="reign-text-small reign-text-muted reign-text-center">${totalXP} XP distributed to ${awards.length} character${awards.length !== 1 ? "s" : ""}.</p>
      </div>`,
    });
